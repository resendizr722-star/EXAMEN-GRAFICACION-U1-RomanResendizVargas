 ...
 * * Programador: Roman Resendiz Vargas | Ingeniería en Sistemas Computacionales
 * M.C Víctor Manuel Pinedo Fernández - Graficación
 */
...
// --- CUERPO PRINCIPAL (Tronco y lomo) ---
...
{ type:'circle', x:190, y:350, r:55, color:'#FDD835' }, // Esclerótica
{ type:'circle', x:190, y:350, r:30, color:'#4CAF50' }, // Iris
{ type:'circle', x:190, y:350, r:12, color:'#212121' }, // Pupila
{ type:'poly', pts:[[170,310],[210,310],[190,280]], color:'#FFF176' }, // Detalle decorativo

// --- PATAS Y GARRAS (Delanteras y traseras) ---
{ type: 'poly', pts: [[380,510],[440,490],[410,560],[360,550]], color:'#2E7D32' }, // Pata delantera sup
{ type: 'poly', pts: [[410,560],[360,550],[330,565],[350,595]], color:'#1B5E20' }, // Pata delantera inf
{ type: 'poly', pts: [[720,450],[770,440],[750,540],[700,530]], color:'#1B5E20' }, // Pata trasera sup
{ type: 'poly', pts: [[750,540],[700,530],[670,545],[690,575]], color:'#004D40' }, // Pata trasera inf
{ type: 'poly', pts: [[330,565],[350,595],[310,590]], color:'#FFF59D' },          // Garra delantera
{ type: 'poly', pts: [[670,545],[690,575],[650,570]], color:'#FFF59D' },          // Garra trasera

// --- COLA ENROLLADA (Estructura base y manchas) ---
...
/**
 * Función auxiliar para aclarar u oscurecer colores en formato Hex.
 * Convierte el hex a RGB, suma/resta el offset (amount) y devuelve rgb().
 */
...
// Luz superior filtrada (Efecto God Rays base)
...
// Ramas gruesas desde bordes (Curvas Bézier)
for (let i = 0; i < 6; i++) {
...
// Hojas grandes desde los bordes (Curvas Cuadráticas)
for (let i = 0; i < 18; i++) {
...
// Hojas pequeñas difusas de fondo (Profundidad de campo)
for (let i = 0; i < 25; i++) {
    ...
    // Luz filtrada (Luces volumétricas secundarias)
    for(let i=0;i<6;i++){
    ...
    chameleonModel.forEach(part=>{
        ...
        if(isEyeCenter){
            ...
            if(part.r === 55){
                ...
                // iris (r=30) con gradiente radial y micro-brillo
                ctx.beginPath();
                ...
                // trazo oscuro fino alrededor del iris para separación visual
                ctx.strokeStyle = adjustColor('#4CAF50', -70);
                ctx.lineWidth = 3;
                ctx.stroke();

                // pupila (r=12) negra con brillo principal y secundario
                ctx.beginPath();
                ctx.arc(190,350,12,0,Math.PI*2);
                ctx.fillStyle = "#0B0B0B";
                ctx.fill();

                // brillo especular principal en la pupila
                ctx.beginPath();
                ctx.fillStyle = "rgba(255,255,255,0.9)";
                ctx.arc(184,344,4,0,Math.PI*2);
                ctx.fill();

                // pequeño reflejo secundario en la pupila
                ctx.beginPath();
                ctx.fillStyle = "rgba(255,255,255,0.18)";
                ctx.arc(198,356,6,0,Math.PI*2);
                ctx.fill();

                // triángulo decorativo inferior del ojo
                ...
                if(tri){
                    ...
                    // relleno suave con gradiente lineal
        ...
        // Flujo normal para todas las demás piezas (Cuerpo, Cola, etc.)
        ctx.beginPath();

        // Variables para calcular los límites de la figura (Bounding Box) y aplicar el degradado
        ...
        // Aplicación del degradado lineal dinámico (Luz a sombra)
        ...
        // Contorno unificador Pop Art (Trazo grueso)
        ctx.strokeStyle="#000";
        ctx.lineWidth=5;
        ctx.lineJoin="round";
        ctx.lineCap="round";
